...
class DatabaseSettingsCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        $this->variables['DB_USERNAME'] = $this->option('username') ?? $this->ask(
            'Database Username',
            config('database.connections.mysql.username', 'pteranodon')
        );
        ...
        try {
            ...
            if ($this->confirm('Go back and try again?')) {
                $this->database->disconnect('_pteranodon_command_test');
    ...
    {
        config()->set('database.connections._pteranodon_command_test', [
            'driver' => 'mysql',
        ...
        $this->database->connection('_pteranodon_command_test')->getPdo();
    }
}
