...
@section('footer-scripts')
    ...
    $(document).ready(function () {
        $('#pEggId').select2({placeholder: 'Select a Nest Egg'}).on('change', function () {
            ...
            var parentChain = _.get(Pteranodon.nests, $("#pNestId").val());
            ...
            for (let i = 0; i < keys.length; i++) {
                ...
                if (objectChain.id === parseInt(Pteranodon.server.egg_id) && Pteranodon.server.image == opt.value) {
                    opt.selected = true
                }
            ...
            if (objectChain.id === parseInt(Pteranodon.server.egg_id)) {
                if ($('#pDockerImage').val() != Pteranodon.server.image) {
                    $('#pDockerImageCustom').val(Pteranodon.server.image);
                }
            }
            ...
            $.each(_.get(objectChain, 'variables', []), function (i, item) {
                var setValue = _.get(Pteranodon.server_variables, item.env_variable, item.default_value);
        ...
        $('#pNestId').select2({placeholder: 'Select a Nest'}).on('change', function () {
            $('#pEggId').html('').select2({
                data: $.map(_.get(Pteranodon.nests, $(this).val() + '.eggs', []), function (item) {
                    return {
                        id: item.id,
                        text: item.name,
                    };
            ...
            if (_.isObject(_.get(Pteranodon.nests, $(this).val() + '.eggs.' + Pteranodon.server.egg_id))) {
                $('#pEggId').val(Pteranodon.server.egg_id);
            }
